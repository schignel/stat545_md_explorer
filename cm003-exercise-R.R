...
commute <- c(my_commute, 20, 31, 45, 60)
commute
...
(diffs <- commute - mu) # R automatically loops through each value in the vector, subtracting the mean from each one. Very powerful!

## Finish computing variance: add up the square of `diffs`, divide by `n-1`.
##  Store in the variable `variance`.
variance <- sum(diffs^2) / (n-1) #Note that R recognizes order of operations, so n-1 need brackets
variance
...
"hi" == "hi"
typeof(TRUE) #Confirms that == returns a logical object ("TRUE")
