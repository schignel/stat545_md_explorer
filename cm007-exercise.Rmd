...
gapminder %>% 
  ...
  scale_x_log10() +
  geom_point()+
  geom_path(arrow=arrow()) #connects the dots in the order they appear in the data frame so we can see how lifeExp vs GDP has changed through time (arrang(year))
```
...
```{r}
ggplot(mtcars, aes(factor(cyl), factor(am))) + #force cyl and am to be factor categories using factor()
  geom_bin2d()
```
