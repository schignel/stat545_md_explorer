...
colnames(airlines)
colnames(flights2)
left_join(flights2,airlines) #didn't have to specify because R found the common column name
```
...
intersect(y,x) # These don't work because the data sets don't have matching column names
union(y,x) 
```
...
```{r}
bind_cols(y,z)
#notice that dplyer added "1" to the column "x2" in order to avoid duplicate names
```
...
```{r}
intersect(flights2, weather) #this test shows that no, we can't use `set` operations because the two datasets don't share exactly the same colum names.

bind_rows(flights2, weather) #however, we can use binding functions, but you just get NA
...
```{r duplicate, cache = T}
(p <-  tibble(x1 = c("a","b","b","c"), x2 = 1:4)) #b is present twice
(q <- tibble(x1 = c("a","b","c","c"), x3 = 5:8)) #c is present twice

left_join(p,q) #notice that it's not dropping anything
left_join(q,p) #notice that it's not dropping anything
```
