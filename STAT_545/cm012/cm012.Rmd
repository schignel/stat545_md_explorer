...
```{r}
#setting the seed ensures the results of the randomization is the same everytime. (this was done so we all get the same values in class, but is not necessary)
set.seed(10) 
...
draw <- factor(draw)
draw
levels(draw) # levels are the possible unique categories
nlevels(draw)
fct_count(draw)
```
...
```{r}
#c(lotr1$Film, lotr2$Film) #doesn't work cith c()
fct_c(lotr1$Film, lotr2$Film) #it works with forcat package! 
```
...
```{r}
rbind(lotr1, lotr2) #rbind is more lenient (makes assumptions)
bind_rows(lotr1, lotr2) # bind_rows is more conservative
```
...
ggplot(gap_small, aes(continent)) +
  geom_bar()+
  scale_x_discrete(drop=FALSE) # keeps the factor levels that don't have values.
...
cont %>% 
  ...
  fct_rev() %>%  #decreasing (just delete to go back to increasing)
  qplot()
```
...
gap_asia_2007 %>% 
  mutate(country = fct_reorder(country, lifeExp)) %>% # orders the factor levels (and resulting ggplot) points by lifeExp
...
gap_asia_2007$country %>% 
  fct_relevel("Syria", "Sweden", after=2) %>% # NB VCoia said Sweden is only in this Asia group because he it's an undropped level
  levels() %>% 
  head()
```
...
cont %>% 
  fct_lump(n=2) %>% #n=2 says: keep the top 2 levels and lump everything else into "Other". fct_lump() by default chooses the n that makes the "Other" category less than the n categories.
  qplot()
```
...
```{r}
lubridate::ymd(170511) #ymd is dataset that comes with lubridate package
lubridate::ymd("2017-May-11") #works with many date formats
```
