...
How many rows within each continent group?
```{r}
gapminder %>% 
  group_by(continent) %>% 
  #summarise(num = n())
  tally() #does the same thing as above line

gapminder %>%
  count(continent) #does the same thing as tally
```
...
gapminder %>% 
  group_by(country) %>% 
  mutate(growth = pop - pop[year == 1972]) # [] subsets by a something. Could alternatively use if_else()
```
...
gapminder %>% 
  ...
  filter(!is.na(gain)) %>%  # NOT NA values
...
# The more sane  wayto use a Table Join (will go over that later)
```
