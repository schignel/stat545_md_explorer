...
```
They tend to require data in tidy format
...
i.e., predict the lefthand side base on the variables on the righthand side.
...
unclass(my_lm)
names(my_lm) #check out what components (model outputs) are available
my_lm$coefficients #look at the coefficients
```
...
```{r}
predict(my_lm) %>% #takes a dataframe, and for each row it takes the x value and predicts a corresponding y value. Needs tidy data. 
  head()

# In order to predict to new data, you use the `newdata` argument
predict(my_lm, newdata = tibble(gdpPercap = c(500, 600, 700))) %>% #takes a dataframe, and for each row it takes the x value and predicts a corresponding y value. Needs tidy data. 
  head()
```
...
```{r}
tidy(my_lm) # note the low p.value shows there is a significant linear relationship
augment(my_lm) #adds the model outputs as columns
augment(my_lm, newdata = tibble(gdpPercap = c(500, 600, 700))) #can add newdata
glance(my_lm) #how well the model is fit
```
