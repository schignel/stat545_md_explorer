...
```{r}
x <- 1:10
sqrt(x) # You could do this. OR you can use map
map(x, sqrt)
```
...
```{r}
# Can define the function on the fly:
map(x, function(w) w^2)

# Or could have defined the function on the line above:
...
# This will do the computations and coerce the outputs as a chr
map_chr(x, sqrt)
```
...
## Method 3:
map_chr(wesanderson, `[`,1)  # using the subsetting function 
```
...
# unname() function removes vector names if you want that
```
...
```{r}
desired_info <- c("name", "born") # create variable with column name
map(got_chars, `[`, desired_info) # pull out the info we want
map_dfr(got_chars, `[`, desired_info) # put it into a data.frame
```
...
# The . is everything to the left of the pipe
# squiggle brackets kind of tricks R
# The very last line in {} is what gets returned
```
