...
```{r}
# Not that important of a step, but can be useful to get a sense of the structure
...
Use ScrapeMate Beta for Firefox.

1. Use the gadget to select the songs. 
2. Put the CSS selection in the `html_nodes()`.

```{r}
read_html("https://musixmatch.com/search/kane%20brown") %>% 
...
read_html("https://en.wikipedia.org/wiki/List_of_roller_coaster_rankings") %>% 
  ...
  .[[2]] #select just the rollercoaster table
```
