...
```{r}
# Making a function that squares the input
square <- function(x) x^2
square(10)
```

```{r}
# Square brackets allow for multiple lines
square <- function(x) {
  y <- x^2
  y
}
...
```{r}
# The return() function will stop the function at that line
square <- function(x) {
  y <- x^2
  return(y)
  5 # this line is not being used because of the return()
}
square(10)
...
# runif() creates a vector of 100 uniform random variables between 0-1
```

Let's formalize this testing with the `testthat` package. `expect_*()` functions:

```{r}
expect_equal(0.1 + 0.2, 0.3) # Argument 1 should equal Argument 2
expect_identical(0.1 + 0.2, 0.3) 

# The test breaks and expect() throws an error. If the test passes, it will return nothing. No news is good news!
```

Add another check to the following unit test, based on the uniform random numbers:

```{r}
# Notice how readable the code is! The label will be returned on an error
...
# It gives an error because it doesn't know how to handle the NAs
```
